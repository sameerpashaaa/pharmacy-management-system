...
import { parseDaysThreshold, parseReportPagination } from '@/lib/reports/report-params'
...
export async function GET(req: NextRequest) {
  try {
    ...
    const daysThreshold = parseDaysThreshold(query.daysThreshold)
    const { page, limit } = parseReportPagination(query)
    const result = await ReportService.getNearExpiry(branchId, daysThreshold, { page, limit })
    return NextResponse.json({
      success: true,
      data: result.data,
