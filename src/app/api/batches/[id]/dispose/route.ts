import type { NextRequest } from 'next/server'
import { NextResponse } from 'next/server'
import { ZodError } from 'zod'

import { requirePermission } from '@/lib/auth/auth-helpers'
import { disposeBatch } from '@/lib/batches/batch-service'
import { PERMISSIONS } from '@/lib/constants/permissions'
import { prisma } from '@/lib/db/prisma'
import { disposeBatchSchema } from '@/lib/validations/batch'

type RouteParams = { params: { id: string } }

// POST /api/batches/:id/dispose
export async function POST(req: NextRequest, { params }: RouteParams) {
  try {
    const user = await requirePermission(PERMISSIONS.BATCHES_DISPOSE)

    const body: unknown = await req.json()
    const data = disposeBatchSchema.parse(body)

    const batch = await disposeBatch(params.id, data, user)

    // Audit
    await prisma.auditLog.create({
      data: {
        userId: user.id,
        action: 'DISPOSE',
        entity: 'Batch',
        entityId: params.id,
        metadata: { quantity: data.quantity, reason: data.reason },
      },
    })

    return NextResponse.json({
      success: true,
      data: batch,
      message:
        batch.status === 'DISPOSED'
          ? 'Batch fully disposed'
          : `${data.quantity} unit${data.quantity === 1 ? '' : 's'} disposed from batch`,
    })
  } catch (err) {
    if (err instanceof ZodError) {
      return NextResponse.json(
        {
          success: false,
          error: {
            code: 'VALIDATION',
            message: err.errors[0]?.message ?? 'Invalid input',
            issues: err.flatten(),
          },
        },
        { status: 400 }
      )
    }
    const message = err instanceof Error ? err.message : 'Unknown error'
    const status =
      message === 'Unauthorized'
        ? 401
        : message.startsWith('Forbidden')
          ? 403
          : message.startsWith('Not Found')
            ? 404
            : message.startsWith('Conflict')
              ? 409
              : message.startsWith('Insufficient')
                ? 400
                : 400
    return NextResponse.json({ success: false, error: { code: 'ERROR', message } }, { status })
  }
}
