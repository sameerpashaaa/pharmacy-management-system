...
import { inventoryListQuerySchema } from '@/lib/validations/inventory'

// GET /api/inventory?page=&limit=&search=&branchId=&productId=&status=&sortBy=&sortOrder=
export async function GET(req: NextRequest) {
  try {
    const user = await requirePermission(PERMISSIONS.INVENTORY_READ)

    const { searchParams } = new URL(req.url)
    const query = inventoryListQuerySchema.parse(Object.fromEntries(searchParams))
