...
**Batch Management:** lifecycle via `src/lib/batches/batch-service.ts` (create/update/block/dispose, lazy `expireDueBatches()` sweep, `BatchStatusLog`); list/detail/block/dispose APIs (`src/app/api/batches/**`); `/batches` server pages. No `batches:create` permission/seeds (creation arrives via future GRN).

**FEFO selection (reusable):** `src/lib/batches/fefo.ts` (pure eligibility/ordering/allocation) consumed by `src/lib/batches/fefo-service.ts`. Read-only advisory; POS/dispensing wiring is Phase 3.

**Expiry Detection:** `validate (src/lib/validations/expiry.ts) → expiry-service.ts (getExpiringBatches / getExpiredBatches / getExpirySummary) → prisma.batches`, always after `expireDueBatches()`; GET `/api/expiry/expiring` + `/api/expiry/expired` (permission `batches:read`, branch scope); server pages `/expiry`, `/expiry/expiring`, `/expiry/expired`.

**POS / Sales:** `validate (src/lib/validations/sale.ts) → sales-service.ts createSale (Serializable tx + P2033/P2034 retry) → prisma (sale + sale_items + sale_item_batches + payments + inventory_movements OUT/SALE + batch CAS→EXHAUSTED + BranchStatusLog + auditLog + per-branch invoiceCounter CAS)`. Read paths: `searchPosProducts` (name/sku/barcode/additionalBarcodes per branch), `listSales`/`getSaleById` (branch scope), held bills (`/api/pos/held-bills`). Money is **never** accepted from the client — every price/tax/total is recomputed server-side from `pricing.ts` using DB product rows + `getPosSettings` (tax mode, round-off, max discount, credit/customer policy). UI: `/pos` (billing grid + cart + payment dialog + held bills + printable receipt), `/sales` history + `/sales/[id]` detail.
...
- **Batch Management** — ✅ SHIPPED (§5; §14 D7). **FEFO selection service** — ✅ SHIPPED (`src/lib/batches/fefo.ts` + `fefo-service.ts`). **Expiry Detection** — ✅ SHIPPED (§5, §14 D9): expiring (30/60/90-day) + expired views + hub, reuses lazy sweep + `batches:read`. **POS wiring of FEFO** — ✅ SHIPPED in `createSale` (Phase 3, §5/§14 D10): sell-time allocation via `allocateFefo`/`allocateByCreationDate` with transactional CAS deduction, expiry-aware (ACTIVE + `expiryDate >= now` only).
- **POS / Sales / Billing** — ✅ SHIPPED (Phase 3, §5/§14 D10): transactionally recorded `sales/sale_items/sale_item_batches/payments/held_bills`; server-side pricing; discount/credit/prescription gates; held bills; printable receipt. Not implemented: A4/thermal PDF generation, returns/refunds (Phase 5), credit-note application, customer-ledger posting.
...
| 4   | FEFO dispensing policy                                                                                         | `createSale` (Phase 3, §14 D10) allocates sell-time via `allocateFefo` (or `allocateByCreationDate` when `fefo_enabled` off), deducting batch stock with optimistic CAS; only ACTIVE + non-expired batches dispense-eligible. Optional batch override w/ justification **FUTURE** | **CONSISTENT**                                                           |
...
| 8   | Near-expiry <30 days red highlight                                                                             | Expiry views shipped: expiring page classifies ≤30 CRITICAL (red/destructive badge), ≤60 WARNING, ≤90 INFO; expired blocked from dispense per FEFO/Batch rules                                                                                                                    | **CONSISTENT**                                                           |
...
| 6   | Batch management & FEFO                                          | **A**                       | Batch lifecycle + FEFO selection shipped; POS sell-time auto-select + CAS deduction now shipped (Phase 3, §14 D10)                                                                                                                                              |
| 7   | Expiry detection & alerts                                        | **A (views) / F (alerts)**  | Detection shipped: lazy sweep flips `BatchStatus=EXPIRED`; expiring (≤30/≤60/≤90) + expired views + hub; POS dispense blocks expired batches. SMS/email/notification alerting remains **F**                                                                     |
...
| 18  | POS / sales / invoices / held bills                              | **A**                       | POS shipped (Phase 3): transactional sales, server-side pricing, FEFO/CAS stock deduction, discount/credit/prescription gates, held bills, printable receipt (browser print). A4/thermal PDF generation, returns/refunds, credit-note application remain future |
...
| V&V Report               | Claims >85% coverage, pen-test done, k6 200 users, FEFO 10 scenarios, tamper-proof audit trail, all "Pass"                                                             | No k6, no pen-tests logged, no FEFO, no hash-chained audit trail; 194 Jest tests at baseline — now **308** (no coverage threshold enforced)                         | **C** — unverified claims                                              |
...
- **Phase 3 POS was implemented as its own controlled task** (commit `feat: implement POS sales and billing`); it **shipped** server-authoritative pricing/credit/discount/prescription gates, FEFO/CAS stock consumption, held bills and the billing UI. It deliberately did **not** touch Purchases/GRN, Prescriptions, Returns, Finance, or Reporting. See §14 D10.
- **No source changes were made by this reconciliation.** Future controlled tasks (not this one) must implement: Purchases/GRN, POS, Prescriptions/Returns, notifications, deployments. Batch, FEFO, and Expiry Detection are now shipped (see §14 D7/D8/D9).
...
### D9 — Expiry Detection implementation (Sept 2026) — `RESOLVED — SHIPPED`

- **Scope shipped** (commit `feat: implement expiry detection`): `GET /api/expiry/expiring` + `GET /api/expiry/expired` (reuse `batches:read` + `resolveBranchScope`), service `src/lib/batches/expiry-service.ts`, server pages `/expiry` (hub with severity + expired summary cards), `/expiry/expiring`, `/expiry/expired`, validation in `src/lib/validations/expiry.ts`, client tables/filters/pagination in `src/components/expiry/*`. Tests: 32 added (service 22, routes 6 + 4); suite now **308 passing / 29 suites**.
- **Thresholds (from docs, verified):** `Batch_Expiry_Tracking.md` §3 → 90d Info, 60d Warning, 30d Critical; `FAQ_Troubleshooting_Guide.md`; `Data_Flow_Diagrams_DFD.md` L2 (`WHERE expiry_date <= NOW() + 90 days`); `Test_Cases_Test_Scripts.md` TC-EXP-001/002. Implemented as inclusive boundaries: `daysRemaining = ceil((expiryDate - now)/1 day)`; `≤30` CRITICAL, `≤60` WARNING, `≤90` INFO, `>90` excluded, `≤0` = expired.
- **Expiring view semantics:** status `ACTIVE`, available quantity `> 0`, expiry strictly after `now` and `≤ now + 90d`. Severity filter + pagination are in-memory (classification is derived from `now`, not stored). ORDER: `expiryDate asc, id asc` (same deterministic tie-break as FEFO).
- **Expired view semantics:** status `EXPIRED` **only**, kept current by reusing the single lazy `expireDueBatches()` before every read — the same expiry mechanism as Batch/FEFO §4.4. Oldest-expired first.
- **Explicitly out of scope (not implemented, matches task):** near-expiry **alerting** (no SMS/email/cron/notifications), any `projects`/schema change, new permissions, POS/Purchases/Returns/Finance changes, `storage_condition` (still D3/future), `/batches/expiring` orphan stub (left untouched, superseded by `/expiry/expiring`).
- **D8 cross-reference:** expiry severity uses the same eligibility family as FEFO (ACTIVE + available > 0 + not-yet-expired); a batch that FEFO would select is exactly a batch the expiring view can show, and vice-versa for expired — consistent surfaces over one model.

### D6 — Cross-cutting note

Only **D2** remains `UNRESOLVED` (owner decision). D1/D5 are documentation corrections; D3/D4 are scheduled future implementation; D7 (Batch), D8 (FEFO), D9 (Expiry Detection) and D10 (POS) are shipped. None of the decision items blocked Batch/FEFO/Expiry/POS delivery.

### D10 — POS / Sales implementation (Sept 2026) — `RESOLVED — SHIPPED`

- **Scope shipped** (commit `feat: implement POS sales and billing`): transactional `createSale` (Serializable + P2033/P2034 retry), pure server-side pricing (`pricing.ts`), FEFO-first (or oldest-first) sell-time batch allocation with optimistic-CAS deduction + `EXHAUSTED` flip + `BatchStatusLog`, inventory aggregate CAS (`updatedAt`) + `OUT`/`SALE` movement, per-branch invoice-counter CAS, policy gates (discount `sales:discount`/`sales:discount_override`, credit `sales:credit`/`allow_credit_sales`/`requireCustomerForCredit`, top-level `prescriptionId` gate for schedule H/X + prescription-only items, same-branch), held bills (save/load/delete over `/api/pos/held-bills`), `/api/pos/config` + product search (name/sku/barcode/additionalBarcodes), POS billing UI (`/pos`) + receipt print, sales history (`/sales`) + detail (`/sales/[id]`).
...
_Maintained by: PharmaCare Development | Baseline generated: September 13, 2026 | Specification decisions record §14 added September 13, 2026 | §14 maintained per feature — D7 (Batch Management) added September 13, 2026; D8 (FEFO selection) added September 13, 2026; D9 (Expiry Detection) added September 13, 2026; D10 (POS / Sales) added September 13, 2026._
